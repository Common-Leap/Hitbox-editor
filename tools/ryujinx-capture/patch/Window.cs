    ...
    public class Window
    {
        ...
        public void Present(Action swapBuffersCallback)
        {
            ...
            if (_frameQueue.TryDequeue(out PresentationTexture pt))
            {
                ...
                if (EffectCapture.FrameDumpDue())
                {
                    try
                    {
                        // Hook 3: read the presented color target back for the capture-diff
                        // harness. HostTexture is at host resolution (ScaleFactor applied).
                        using var data = texture.HostTexture.GetData();
                        int dumpWidth = (int)MathF.Ceiling(texture.Info.Width * texture.ScaleFactor);
                        int dumpHeight = (int)MathF.Ceiling(texture.Info.Height * texture.ScaleFactor);
                        EffectCapture.DumpFrame(
                            EffectCapture.CurrentFrame,
                            dumpWidth,
                            dumpHeight,
                    ...
                    {
                        System.Console.Error.WriteLine($"[EffectCapture] present readback failed: {e.Message}");
                    }
                }
